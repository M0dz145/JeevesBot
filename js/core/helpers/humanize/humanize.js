export default class Humanize {
    ...
    static intComma(number, decimals = 0) {
        return this.formatNumber(number, decimals);
    }

    static intcomma(...args) {
        return this.intComma(...args);
    }

    // Formats the value like a 'human-readable' file size (i.e. '13 KB', '4.1 MB', '102 bytes', etc).
    static fileSize(filesize, precision = 2) {
    ...
    static filesize(...args) {
        return this.fileSize(...args);
    }

    // Formats a number to a human-readable string.
    // Localize by overriding the precision, thousand and decimal arguments.
    static formatNumber(number, precision = 0, thousand = ',', decimal = '.') {
        ...
        const decimals = (_number, _decimal, usePrecision) => {
            return usePrecision
                ? _decimal + this.toFixed(Math.abs(_number), usePrecision).split('.')[1]
                : '';
        };

        const usePrecision = this.normalizePrecision(precision);

        // Do some calc
        const negative = number < 0 && '-' || '';
        const base = String(parseInt(this.toFixed(Math.abs(number || 0), usePrecision), 10));
    ...
    static normalizePrecision(value, base) {
    ...
    static pluralize(number, singular, plural) {
    ...
    static truncateWords(string, length) {
    ...
    static truncatewords(...args) {
        return this.truncateWords(...args);
    }

    // Truncates a number to an upper bound.
    static boundedNumber(num, bound = 100, ending = '+') {
        let result;
    ...
    static truncatenumber(...args) {
        return this.boundedNumber(...args);
    }

    // Converts a list of items to a human readable string with an optional limit.
    static oxford(items, limit, limitStr) {
    ...
    static dictionary(object, joiner = ' is ', separator = ', ') {
        const result = '';
    ...
    static frequency(list, verb) {
        if(!isArray(list)) {
            return null;
        }

        const len = list.length;
        const times = this.times(len);
    ...
    static pace(value, intervalMs, unit = 'time') {
    ...
    static nl2br(string, replacement = '<br/>') {
        return string.replace(/\n/g, replacement);
    }

    // Converts <br/> tags to newlines
    static br2nl(string, replacement = '\r\n') {
        return string.replace(/\<br\s*\/?\>/g, replacement);
    }

    // Capitalizes first letter in a string
    static capitalize(string, downCaseTail = false) {
    ...
    static titlecase(...args) {
        return this.titleCase(...args);
    }
